...
import {FileInputField, InputField, TextAreaInputField} from "../../config/FormElements";
...
const OrderShow = () => {
    ...
    const sendFeedback = () =>{
        ...
        axios.post('/api/auth/client/feedback', {rating:rating , remarks:feedback , order_id:order.id} )
            .then(res => {
                if(res.status == 201){
                     ...
                     <div className="order-preview">
                         ...
                         {(order && (order.stage == 1 && !order.feedback)) && (
                             <div className="unassigned-order-actions py-4 mb-3 bg-gradient-to-r from-indigo-700 to-indigo-500 hover:from-indigo-100 hover:to-indigo-100 rounded-md w-full transition-all delay-75 flex  justify-center items-center">
                                 <span className="text-white mr-2">Please Give Feedback On This Service!</span>
                                 <button className="text-indigo-600 bg-white px-4 py-2 rounded hover:bg-indigo-600 hover:text-white" onClick={(e) =>{
                                       e.preventDefault();
