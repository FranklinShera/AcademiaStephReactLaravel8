...
const OrderShow = () => {
    ...
    const getOrder = (orderID) => {
        axios
            .get(`/api/auth/client/order/${orderID}`)
            .then((res) => {
    ...
    const deleteMaterial = (materialId) => {
        axios
            .delete("/api/auth/client/material/" + materialId)
            .then((res) => {
                if (res.status == 200) {
                    window.Toast.fire({
                        icon: "success",
    ...
    const submitMaterialForm = (formFields) => {
        ...
        axios
            .post(
                "/api/auth/client/add-material/" + order.id,
                materialFormData,
                {
                    headers: {
    ...
    const cancelOrder = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .post(`/api/auth/client/cancel-order/${order.id}`)
                    .then((res) => {
                        if (res.status == 200) {
                            Swal.fire(
                                "Cancelled!",
                ...
                <div className="dash_overview">
                    <div className="preview-order-page">
                        ...
                        <h4 className="mt-6 text-lg text-primary-4">
                            Posted On {order && order.created_at_date} at{" "}
                            {order && order.created_at_time}
                        </h4>
                        ...
                        <div className="order-preview">
                            ...
                            {order && order.stage == 1 && order.feedback && (
                                <div className=" py-4 mb-3 bg-gradient-to-r from-green-600 to-green-400  rounded-md w-full transition-all delay-75 flex  justify-center items-center">
                                    ...
                                    <span className="text-xl font-bold text-white p-1 border border-white rounded-lg">
                            ...
                            {order && order.stage == 1 && !order.feedback && (
                                <div className="unassigned-order-actions py-4 mb-3 bg-gradient-to-r from-indigo-700 to-indigo-500 hover:from-indigo-100 hover:to-indigo-100 rounded-md w-full transition-all delay-75 flex  justify-center items-center">
                                    <span className="text-white mr-2">
                                        Please Give Feedback On This Service!
                                    </span>
                                    <button
                            ...
                            {order && order.stage == 4 && (
                                <div className="unassigned-order-actions py-4 mb-3 hover:bg-red-200 bg-red-600 rounded-md w-full transition-all delay-75 flex  justify-center items-center">
                                    <span className="text-white mr-2">
                                        You May Not Be Able To Cancel This Order
                                        After It's Assigned!
                                    </span>
                                    <button
                            ...
                            {order && order.stage == "0" && (
                                <>
                                    <div className="order-preview-notification">
                                        Pay For This Order For It To Be
                                        Assignable!
                                        <span
                                            className="payaction"
                                            onClick={(e) => {
                                ...
                                {orderMaterials.length != 0 &&
                                    orderMaterials.map((material, index) => (
                                        <div className="material" key={index}>
                                            ...
                                            <a
                                                target="blank"
                                                href={`/storage/order/materials/${material.material_name}`}
                                            >
                                                {" "}
