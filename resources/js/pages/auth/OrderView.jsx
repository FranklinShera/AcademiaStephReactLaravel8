...
const OrderView = () => {
    ...
    const getOrder = (orderID) => {
        axios
            .get(`/api/auth/admin/order/${orderID}`)
            .then((res) => {
        ...
        axios
            .get(`/api/auth/admin/assignable-writers`)
            .then((res) => {
    ...
    const markComplete = (e) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .post(`/api/auth/admin/complete-order/${order.id}`)
                    .then((res) => {
                        if (res.status == 200) {
                            Swal.fire(
                                "Order Completed!",
    ...
    const assignOrder = (e) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .post(
                        `/api/auth/admin/assign-order/${order.id}/${assignedWriter}`
                    )
                    .then((res) => {
                        if (res.status == 200) {
                            ...
                            {order && order.stage == 2 && (
                                <div className="unassigned-order-actions py-4 w-ful flex justify-center mb-3 hover:bg-green-200 rounded-md items-center bg-green-700 text-white">
                                    <span>
                                        ...
                                        {order &&
                                            order.assigned_to?.name.toUpperCase()}
                                    </span>
                                    <button
