...
const PaperType = () => {
    ...
    const addPaperType = async (paperTypeForm) =>{
        ...
        axios.post('/api/auth/admin/paper-type' , paperTypeForm).then(res => {
                ...
                <div className="levels--links--meta">
                    ...
                    <div className="links--fetch">
                        ...
                        <button  className={links.prev ? " bg-primary-3 text-white " : " bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.prev && dispatch(adminFetchPaperTypes(links.prev))
                        }}>PREV</button>

                        <button className={ links.prev ? " bg-primary-3 text-white " : " bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.prev && dispatch(adminFetchPaperTypes(links.first))
                        }}>FIRST</button>

                        <button className={links.next ? " bg-primary-3 text-white " :" bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.next && dispatch(adminFetchPaperTypes(links.last))
                        }}>LAST</button>

                        <button className={links.next ? " bg-primary-3 text-white " :" bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.next && dispatch(adminFetchPaperTypes(links.next))
                        }}>NEXT</button>
