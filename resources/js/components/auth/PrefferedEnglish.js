...
import { adminFetchPrefferedEnglish} from '../../actions/OrderActions'
...
const PrefferedEnglish = () => {
    ...
    const { allPreffEnglish , loading , meta , links} = PreffEnglish;


    let[langChange, setLangChange] = useState(0)
    ...
    const formik = useFormik({
        ...
        validationSchema:Yup.object({
            lang_name: Yup.string()
                .min(2 , 'Lang Cannot Be Less Than 3 Characters')
                .max(32, 'Lang Cannot be More than 32 Characters')
    ...
    const deletePreffEng = async (id) =>{

        setLocalLoad(true)

        const res = await axios.delete('/api/auth/admin/preffered-english/'+ id )
    ...
    const toggleStatus = (id) =>{
        ...
        axios.post('/api/auth/admin/preffered-english-toggle/'+id).then(res =>{
            if(res.status == 200){
    ...
    const addPreffLang = async (langForm) =>{

        setLocalLoad(true)


        axios.post('/api/auth/admin/preffered-english' , langForm).then(res =>{
    ...
    useEffect(() => {
        ...
        dispatch(adminFetchPrefferedEnglish())
        ...
        <div className="table-group">
            <div className="core-table">
                <div className="core--list--table">
                    ...
                    <div className="core--table--body">
                        ...
                        { (loading || localLoad ) ? <DotLoader/>  :   allPreffEnglish.map((prefferedLang,index) => (
                                <div className="table--item" key={index}>
                                    ...
                                    <div className="table--item--status">
                                        ...
                                        { (prefferedLang.active) ? <span>ACTIVE</span> : <span>INACTIVE</span> }
                                        </span>

                                        <span className="table--item--actions">

                                             <i className={`ti-exchange-vertical cursor-pointer ${ prefferedLang.active ? 'text-palblue' : 'text-dark-1'}`}
                                                onClick={(e) => {
                                                    e.preventDefault()
                                                    toggleStatus(prefferedLang.id)
                                                }}>
                                            ...
                                            <svg onClick={(e) => {
                                                e.preventDefault()
                                                deletePreffEng(prefferedLang.id)
                                            }}
                        ...
                        {(allPreffEnglish.length == 0 && !loading) && (
                            <div className="no--table--items">
                                <h1>We Could'nt Find Any Preffered Languages (Eng)!</h1>
            ...
            <div className="new--table--item">
                <form action="" onSubmit={formik.handleSubmit}>
                    ...
                    <InputField
                        labelText='Preffered English'
                        name="lang_name"
                        type='text'
                        onBlur={formik.handleBlur}
                        value={formik.values.lang_name}
                        placeholder="Add Preffered English entry ..."
                        onChange={formik.handleChange}
                        errors={(formik.errors.lang_name && formik.touched.lang_name) && formik.errors.lang_name}
                    />
                ...
                <div className="table--links--meta">
                    ...
                    <div className="links--fetch">
                        ...
                        <button  className={links.prev ? " bg-primary-3 text-white " : " bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.prev && dispatch(adminFetchPrefferedEnglish(links.prev))
                        }}>PREV</button>

                        <button className={ links.prev ? " bg-primary-3 text-white " : " bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.prev && dispatch(adminFetchPrefferedEnglish(links.first))
                        }}>FIRST</button>

                        <button className={links.next ? " bg-primary-3 text-white " :" bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.next && dispatch(adminFetchPrefferedEnglish(links.last))
                        }}>LAST</button>

                        <button className={links.next ? " bg-primary-3 text-white " :" bg-gray-100 text-gray-400 "} onClick={(e) =>{
                            e.preventDefault();
                            links.next && dispatch(adminFetchPrefferedEnglish(links.next))
                        }}>NEXT</button>
...
export default PrefferedEnglish
